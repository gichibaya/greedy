...
def maxSubArray(a):
    ...
    for i in range(len(a)):
        if a[i] < minSoFar: 
            ...
            maxDif = max(maxDif, a[i] - minSoFar)
    return maxDif
...
def polynomial_multiply(a_coeff_list, b_coeff_list):
    ...
    a_coeff_list = a_coeff_list + [0] * (degree - len(a_coeff_list) + 1)
    b_coeff_list = b_coeff_list + [0] * (degree - len(b_coeff_list) + 1)
    
    fft_a_coeff_list= fft(a_coeff_list)
    fft_b_coeff_list= fft(b_coeff_list)
    newFFT = fft_a_coeff_list * fft_b_coeff_list
    newFFT = ifft(newFFT)
    return newFFT
...
from numpy.fft import fft, ifft
from numpy import real,imag,arange,concatenate
...
fft_frequencies = concatenate((f0, f1, fmid, f2))
...
upto_1_year = []
one_year_to_1_quarter = []
less_than_1_quarter = []

#mentioned above 

upto_1_year = select_all_items_in_freq_range(0, 1/52)# All signal components with frequency < 1/52
one_year_to_1_quarter = select_all_items_in_freq_range(1/52,1/13) # All signal components with frequency between 1/52 (inclusive) and 1/13 weeks (not inclusive)
less_than_1_quarter = select_all_items_in_freq_range(1/13, float('inf')) # All signal components with frequency >= 1/13 
